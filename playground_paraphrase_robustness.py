...
# --- main loop -------------------------------------------------------------
print("=" * 70)
print(f"Target chunk (the one we want to surface):")
print(f"  {TARGET_ID}")
...
for q in QUERIES:
    ...
    if bm25_target_rank == 1 and cos_target_rank == 1:
        print(f"  → Robustness: STABLE on both methods. ✓")
    elif cos_target_rank == 1 and bm25_target_rank is None:
        print(f"  → Robustness: COSINE rescues a BM25 miss. The vector 'gets' the paraphrase.")
    elif cos_target_rank == 1 and bm25_target_rank and bm25_target_rank > 1:
        print(f"  → Robustness: BM25 partial miss, cosine wins.")
    elif cos_target_rank is None:
        print(f"  → Robustness: BOTH methods miss — chunk is below the fold for this query.")
    else:
        print(f"  → Robustness: see numbers above.")
